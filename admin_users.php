...
?>
<style>
.admin-users-grid{display:grid;grid-template-columns:minmax(320px,430px) minmax(0,1fr);gap:20px}
.admin-user-inline{background:#f8fbff;border:1px solid #d9e4f2;border-radius:20px;padding:18px;margin:12px 0}
.admin-user-inline .inline-grid-2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.admin-link-search-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center}
.admin-link-search-result{display:flex;justify-content:space-between;align-items:center;border:1px solid #e5e7eb;border-radius:16px;padding:12px 14px;background:#fff;margin-top:10px;gap:12px}
.admin-link-search-result strong{display:block}
.admin-current-link{font-weight:700;margin:8px 0 12px}
.admin-inline-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}
.admin-selected-pill{display:inline-flex;align-items:center;gap:8px;background:#eef6ff;border:1px solid #cfe2ff;border-radius:999px;padding:8px 12px;font-weight:600;margin-top:10px}
.admin-users-table td{vertical-align:top}
.admin-users-table .btn{margin-right:8px}
.admin-user-cell small{display:block;color:#64748b}
.admin-edit-btn{white-space:nowrap}
@media (max-width:960px){
    .admin-users-grid{grid-template-columns:1fr}
    .admin-user-inline .inline-grid-2,
    .admin-link-search-row{grid-template-columns:1fr}
}
</style>
        ...
        <?php foreach ($users as $user): ?>
          <tr id="user-row-<?= (int)$user['ID'] ?>">
            <td class="admin-user-cell">
              ...
              <?php if (!empty($user['linked_person_name'])): ?>
                <small>Linked donor: <?= e((string)$user['linked_person_name']) ?> #<?= (int)$user['person_id'] ?></small>
              <?php endif; ?>
            </td>
                ...
                <div class="admin-user-inline">
                  ...
                  <form method="post" action="admin_users.php?edit=<?= (int)$editUser['ID'] ?>#edit-user-<?= (int)$editUser['ID'] ?>" class="stack">
                    ...
                    <div class="inline-grid-2">
                      <div>
                        <label>Full Name</label>
                        <input type="text" name="edit_name" value="<?= e((string)$editUser['name']) ?>" required>
                      </div>
                      <div>
                        <label>Phone</label>
                        <input type="text" name="edit_phone" value="<?= e((string)($editUser['phone'] ?? '')) ?>">
                      </div>
                    </div>
                    ...
                    <div class="inline-grid-2">
                      <div>
                        <label>City</label>
                        <input type="text" name="edit_city" value="<?= e((string)($editUser['city'] ?? '')) ?>">
                      </div>
                      <div>
                        <label>Preferred Language</label>
                        <select name="edit_preferred_language">
                          <option value="en" <?= (string)$editUser['preferred_language'] === 'en' ? 'selected' : '' ?>>English</option>
                          <option value="ur" <?= (string)$editUser['preferred_language'] === 'ur' ? 'selected' : '' ?>>Urdu</option>
                          <option value="it" <?= (string)$editUser['preferred_language'] === 'it' ? 'selected' : '' ?>>Italian</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div class="inline-grid-2">
                      <div>
                        ...
                        <select name="edit_role">
                          <option value="operator" <?= (string)$editUser['role'] === 'operator' ? 'selected' : '' ?>>Operator</option>
                          <option value="accountant" <?= (string)$editUser['role'] === 'accountant' ? 'selected' : '' ?>>Accountant</option>
                          <option value="admin" <?= (string)$editUser['role'] === 'admin' ? 'selected' : '' ?>>Admin</option>
                        </select>
                      </div>
                      <div>
                        <label>Status</label>
                        <select name="edit_active">
                          <option value="1" <?= (int)$editUser['active'] === 1 ? 'selected' : '' ?>>Active</option>
                          <option value="0" <?= (int)$editUser['active'] !== 1 ? 'selected' : '' ?>>Disabled</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input type="password" name="edit_password" minlength="10" placeholder="Leave blank to keep current password">
                    </div>
                    ...
                    <div>
                      ...
                      <div class="admin-current-link">
                        Current link:
                        <?php if ((int)($editUser['person_id'] ?? 0) > 0): ?>
                          #<?= (int)$editUser['person_id'] ?> · <?= e((string)($editUser['linked_person_name'] ?? 'Linked donor')) ?>
                        <?php else: ?>
                          No donor linked
                        <?php endif; ?>
                      </div>
                      ...
                      <input
                        ...
                        id="selected_person_id_<?= (int)$editUser['ID'] ?>"
                        value="<?= (int)($editUser['person_id'] ?? 0) ?>"
                      >
                      ...
                      <?php if ((int)($editUser['person_id'] ?? 0) > 0): ?>
                        <div class="admin-selected-pill" id="selected_person_label_<?= (int)$editUser['ID'] ?>">
                          Selected donor: #<?= (int)$editUser['person_id'] ?> · <?= e((string)($editUser['linked_person_name'] ?? '')) ?>
                        </div>
                      <?php else: ?>
                      ...
                      <?php if ($editSearch !== ''): ?>
                        <div style="margin-top:12px">
                          <?php if ($linkResults): ?>
                            <?php foreach ($linkResults as $person): ?>
                              <div class="admin-link-search-result">
                                ...
                                <button
                                  type="button"
                                  class="btn"
                                  onclick='selectLinkedPerson(<?= (int)$editUser["ID"] ?>, <?= (int)$person["ID"] ?>, <?= json_encode("#" . (int)$person["ID"] . " · " . (string)$person["name"]) ?>)'
                                >
                                  Select
                                </button>
                              </div>
