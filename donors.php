...
<div class="card donor-list-shell">
  <form method="get" class="donor-search-form-v5">
    ...
    <input type="text" name="q" value="<?= e($search) ?>" placeholder="Search by donor name, phone, city" class="donor-search-input-v5">
  ...
  <div class="donor-record-list">
    <?php foreach ($people as $person): ?>
      ...
      ?>
      <div class="donor-record-card-v5" onclick="window.location.href='person_profile.php?id=<?= (int)$person['ID'] ?>'" tabindex="0" onkeydown="if(event.key==='Enter'||event.key===' '){window.location.href='person_profile.php?id=<?= (int)$person['ID'] ?>'; event.preventDefault();}">
