...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($action === 'update' && $eventId > 0) {
            if ($statusCol) {
                $pdo->prepare('UPDATE events SET name = ?, estimate = ?, description = ?, status = ? WHERE ID = ?')->execute([$name, $estimate, $description, $status, $eventId]);
            } else {
                $pdo->prepare('UPDATE events SET name = ?, estimate = ?, description = ? WHERE ID = ?')->execute([$name, $estimate, $description, $eventId]);
            }
            ...
            if ($statusCol) {
                $pdo->prepare('INSERT INTO events (name, estimate, description, status, uid, created_at) VALUES (?, ?, ?, ?, ?, NOW())')->execute([$name, $estimate, $description, $status, $uid]);
            } else {
                $pdo->prepare('INSERT INTO events (name, estimate, description, uid, created_at) VALUES (?, ?, ?, ?, NOW())')->execute([$name, $estimate, $description, $uid]);
            }
...
$events = $pdo->query('SELECT e.*, COALESCE((SELECT SUM(amount) FROM operator_ledger ol WHERE ol.reference_id = e.ID AND ol.transaction_category = "event"),0) AS collected FROM events e ORDER BY e.ID DESC')->fetchAll();
$editRow = null;
foreach ($events as $event) {
    if ((int)$event['ID'] === $editId) { $editRow = $event; break; }
}
...
<div class="grid-2 event-page-layout">
  <div class="card stack event-form-card">
    ...
    <div class="toolbar">
      ...
      <?php if ($editRow): ?><a class="btn" href="event_page.php">Cancel</a><?php endif; ?>
    </div>
    <form method="post" class="stack">
      <?= csrfField() ?>
      <input type="hidden" name="action" value="<?= $editRow ? 'update' : 'create' ?>">
      <?php if ($editRow): ?><input type="hidden" name="event_id" value="<?= (int)$editRow['ID'] ?>"><?php endif; ?>
      <div><label>Event Name</label><input type="text" name="name" value="<?= e((string)($editRow['name'] ?? '')) ?>" required></div>
      <div><label>Target Amount</label><input type="number" step="0.01" name="estimate" value="<?= e((string)($editRow['estimate'] ?? '')) ?>"></div>
      ...
      <?php endif; ?>
      <button class="btn btn-primary" type="submit"><?= $editRow ? 'Update Event' : 'Save Event' ?></button>
    </form>
  </div>
  ...
  <div class="card event-list-card">
    ...
    <div class="search-results event-results">
      <?php foreach ($events as $event): ?>
        <a class="search-row event-search-row" href="event_page.php?edit=<?= (int)$event['ID'] ?>">
          <div class="event-row-main">
            ...
            <div class="muted"><?= e((string)$event['description']) ?></div>
            <div class="event-row-stats">
              <span class="tag blue">Target <?= money((float)$event['estimate']) ?></span>
              <span class="tag orange">Collected <?= money((float)$event['collected']) ?></span>
            </div>
          </div>
          <div class="compact-actions event-row-actions">
            <?php if ($statusCol): ?><span class="tag <?= (int)$event['status'] === 1 ? 'green' : 'red' ?>"><?= (int)$event['status'] === 1 ? 'Active' : 'Inactive' ?></span><?php endif; ?>
      ...
      <?php endforeach; ?>
      <?php if (!$events): ?><div class="muted">No events found.</div><?php endif; ?>
    </div>
  </div>
</div>
