...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $params = [
            $form['name'], $form['phone'], $form['city'], $form['notes'],
        ...
        if ($editing) {
            $stmt = $pdo->prepare('UPDATE people SET name=?, phone=?, city=?, notes=?, life_membership=?, life_membership_amount=?, life_membership_start_date=?, monthly_subscription=?, death_insurance_enabled=?, death_insurance_society_id=?, death_membership_no=?, death_start_date=?, home_country_reference_name=?, home_town_address=?, home_town_phone=?, italy_reference_name=?, italy_reference_address=?, italy_reference_phone=?, religion_sect=?, organ_donor_status=?, updated_by=?, updated_at=NOW() WHERE ID=?');
            ...
            $stmt = $pdo->prepare('INSERT INTO people (name, phone, city, notes, life_membership, life_membership_amount, life_membership_start_date, monthly_subscription, death_insurance_enabled, death_insurance_society_id, death_membership_no, death_start_date, home_country_reference_name, home_town_address, home_town_phone, italy_reference_name, italy_reference_address, italy_reference_phone, religion_sect, organ_donor_status, created_by, updated_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())');
...
<div class="card stack donor-form-card-v5">
    ...
    <form method="post" class="stack compact donor-form-v5">
        ...
        <div class="inline-grid-2 donor-main-grid-v5">
            <div>
                <label>City</label>
                <input type="text" name="city" value="<?= e((string)$form['city']) ?>">
            </div>
            <div>
                <label>Notes</label>
